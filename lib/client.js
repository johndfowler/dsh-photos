...
window.__ModuleLoader__.load({ id: 'dsh-photos', factory: () => {
  ...
  function positionToolbar(tb) {
    ...
    if (r.bottom < 0 || r.top > window.innerHeight) { tb.style.display = 'none'; return; }
    tb.style.display = 'flex';
    tb.style.left = 'auto';
    tb.style.right = Math.max(6, window.innerWidth - r.right) + 'px';
    tb.style.top = Math.max(6, r.top - 42) + 'px';
  }
  ...
  function ensureToolbar() {
    ...
    if (window.visualViewport) window.visualViewport.addEventListener('resize', pos);
