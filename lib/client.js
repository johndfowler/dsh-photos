...
window.__ModuleLoader__.load({ id: 'dsh-photos', factory: () => {
  ...
  function apply() {
    ...
    input.addEventListener('change', function () {
      ...
      input.value = '';
      var files = picked.filter(function (f) { return f && f.type && f.type.indexOf('image/') === 0; });
      if (!files.length) return;
      var ta = findComposer();
      if (!ta) return;
      var dt = new DataTransfer();
      files.forEach(function (f) { dt.items.add(f); });
      // Chrome/Safari accept clipboardData in the ClipboardEvent constructor;
      // Firefox ignores init members it can't assign, leaving clipboardData
      // null — then build the event bare and define the getter by hand.
      var ev;
      try {
        ev = new ClipboardEvent('paste', { bubbles: true, cancelable: true, clipboardData: dt });
        if (!ev.clipboardData) throw new Error('clipboardData missing');
      } catch (e) {
